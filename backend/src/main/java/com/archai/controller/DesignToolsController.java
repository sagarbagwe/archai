...
public class DesignToolsController {
    ...
    public Map<String,Object> chat(@Valid @RequestBody ContextRequest request){
        List<String> components=listOfMaps(map(request.design().get("architecture")).get("nodes")).stream().map(node->String.valueOf(node.get("name"))).limit(8).toList();
    ...
    public Map<String,Object> failure(@Valid @RequestBody FailureRequest request){
        ...
        queue.add(request.failedNodeId());
        while(!queue.isEmpty()){
            String id=queue.remove();
            if(!affected.add(id)) continue;
            queue.addAll(outgoing.getOrDefault(id,List.of()));
        }
        List<String> names=nodes.stream().filter(node->affected.contains(String.valueOf(node.get("id")))).map(node->String.valueOf(node.get("name"))).toList();
    ...
    public Map<String,Object> traffic(@Valid @RequestBody TrafficRequest request){
        ...
        List<String> bottlenecks=listOfMaps(map(request.design().get("architecture")).get("nodes")).stream().filter(node->{String type=String.valueOf(node.get("type")).toLowerCase();return type.contains("database")||type.contains("queue")||type.contains("service");}).limit(6).map(node->String.valueOf(node.get("name"))).toList();
    ...
    public Map<String,Object> interviewEvaluate(@Valid @RequestBody InterviewRequest request){
        int words=request.answer()==null?0:request.answer().trim().split("\\s+").length;
        int score=Math.max(2,Math.min(10,words/10+4));
    ...
    public String markdown(@Valid @RequestBody ExportRequest request){
        Map<String,Object> design=request.design();
        return "# "+value(design,"title","ArchAI design")+"\n\nGenerated system design.\n\n## Architecture\n\n```json\n"+String.valueOf(design.getOrDefault("architecture",Map.of()))+"\n```\n";
    }

    @PostMapping(value="/export/sql",produces=MediaType.TEXT_PLAIN_VALUE)
    public String sql(@Valid @RequestBody ExportRequest request){
        return "-- ArchAI generated schema starter\n-- Review indexes, constraints, and migration safety before production.\nCREATE TABLE IF NOT EXISTS architecture_component (\n  id VARCHAR(128) PRIMARY KEY,\n  name VARCHAR(255) NOT NULL,\n  component_type VARCHAR(80) NOT NULL,\n  technology VARCHAR(255),\n  configuration JSON\n);\n";
    }

    private String contextualAnswer(String question){
        String normalized=question.toLowerCase();
        if(normalized.contains("10x")||normalized.contains("traffic")) return "the stateless tier can scale horizontally, but database writes, queue consumers, and external quotas need explicit headroom.";
        if(normalized.contains("redis")||normalized.contains("cache")) return "a cache can reduce read latency and database load, but needs a clear invalidation policy and a database fallback.";
        if(normalized.contains("kafka")||normalized.contains("queue")) return "asynchronous messaging decouples producers from consumers, but introduces lag, duplicate delivery, ordering, and replay concerns.";
        return "trace the critical request path, quantify its limits, and verify failure and recovery behavior before changing the design.";
    }

    @SuppressWarnings("unchecked")
    private Map<String,Object> map(Object value){return value instanceof Map<?,?>?(Map<String,Object>)value:Map.of();}
    @SuppressWarnings("unchecked")
    private List<Map<String,Object>> listOfMaps(Object value){return value instanceof List<?>?(List<Map<String,Object>>)value:List.of();}
