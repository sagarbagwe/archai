...
import { buildInitialDiagram } from "@/lib/diagram";
import { DiagramState, GenerationEvent, SavedDesign } from "@/lib/domain/design";
import { browserStorage } from "@/lib/storage/storage-service";

const stages = [["requirements","Analyzing requirements"],["estimation","Calculating traffic"],["architecture","Designing architecture"]] as const;
...
export default function DesignPage() {
  ...
  const metrics = useMemo(()=>design ? [["Daily requests",design.estimate?.dailyRequests],["Average RPS",design.estimate?.averageRps],["Peak RPS",design.estimate?.peakRps],["Target latency",`${design.input.expectedLatencyMs} ms`]] : [],[design]);
  function receive(event: GenerationEvent){if(event.type==="stage.started")setProgress(old=>({...old,[event.stage]:"active"}));if(event.type==="stage.completed")setProgress(old=>({...old,[event.stage]:"complete"}));if(event.type==="stage.failed")setProgress(old=>({...old,[event.stage]:"failed"}));}

  async function generate(){
    if(!design||generating)return;setGenerating(true);setError(undefined);setProgress({});controller.current=new AbortController();
    try{const generatedDesign=await streamDesignGeneration(design.input,receive,controller.current.signal);const updated:SavedDesign={...design,status:"generated",updatedAt:new Date().toISOString(),generatedDesign,diagram:buildInitialDiagram(generatedDesign.architecture)};browserStorage().saveDesign(updated);setDesign(updated);}
    catch(cause){if((cause as Error).name!=="AbortError")setError(cause instanceof Error?cause.message:"AI generation failed.");}
    finally{setGenerating(false);}
  }

  function saveDiagram(diagram: DiagramState){if(!design)return;const updated={...design,diagram,updatedAt:new Date().toISOString()};browserStorage().saveDesign(updated);setDesign(updated);}
  ...
  return <main className="mx-auto min-h-screen max-w-7xl px-6 py-8">
    ...
    {architecture&&design.diagram?<ArchitectureCanvas key={design.generatedDesign?.title} diagram={design.diagram} onSave={saveDiagram}/>:<section className="rounded-xl border border-dashed border-[var(--border)] p-12 text-center"><h2>Architecture not generated yet</h2><p className="text-[var(--muted)]">Generate the design to open the interactive workspace.</p></section>}
    <section className="mt-8 rounded-xl border border-[var(--border)] p-6"><h2 className="mt-0">Calculation formulas</h2><div className="grid gap-3 md:grid-cols-2">{Object.entries(design.estimate?.formulas??{}).map(([name,formula])=><div key={name} className="rounded-lg bg-[var(--surface)] p-4"><b className="text-sm">{name}</b><code className="mt-2 block text-sm text-[var(--muted)]">{formula}</code></div>)}</div></section>
  </main>;
}
